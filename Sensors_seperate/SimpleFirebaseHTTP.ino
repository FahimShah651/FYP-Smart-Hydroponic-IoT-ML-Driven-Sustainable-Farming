...
const char* FIREBASE_HOST = "your-firebase-project-id-default-rtdb.your-region.firebasedatabase.app";
const char* FIREBASE_API_KEY = "YOUR_FIREBASE_API_KEY";

// Firebase authentication - Replace with your Firebase credentials
const char* FIREBASE_EMAIL = "your-email@example.com";
const char* FIREBASE_PASSWORD = "your-firebase-password";

// Google Sheets Apps Script URL - Replace with your deployed web app URL
...
void uploadDataToFirebase() {
  ...
  Serial.println("Updating latest_readings to firebase ...");
  ...
  if (httpResponseCode > 0) {
    ...
    // Serial.print("HTTP Response code: ");
    // Serial.println(httpResponseCode);
    Serial.println("SUCCESS:data added to firebase");
  } else {
    Serial.print("Error on HTTP request. Error code: ");
    // Serial.println(httpResponseCode);
    Serial.println("FAILED: Could not add  data");
  }
...
void printData(){
  ...
  Serial.println("Water Humidity: " + String(waterHumidity));
  Serial.println("Water Temperature: " + String(waterTemperature));
  Serial.println("Water Conductivity: " + String(waterConductivity));
  Serial.println("Water pH: " + String(waterPH));
  ...
  Serial.println("EC: " + String(ecValue));
  Serial.println("-------------------\n");
...
void loop() {
  ...
  // Check for control commands from Firebase every 10 milli second (reduced from 5 seconds)
  static unsigned long lastControlCheckTime = 0;
  if (millis() - lastControlCheckTime >= 10) {
    checkControlCommands();
    lastControlCheckTime = millis();
  }
  
  // Update relay states in Firebase every 10 milli seconds
  static unsigned long lastRelayUpdateTime = 0;
  if (millis() - lastRelayUpdateTime >= 10) {
    updateRelayStates();
    lastRelayUpdateTime = millis();
  }
  
  // Run auto control logic every 10 milli seconds
  static unsigned long lastAutoControlTime = 0;
  if (millis() - lastAutoControlTime >= 10) {
    runAutoControl();
    lastAutoControlTime = millis();
  }
  
  // Read sensors and upload data every 10 milli seconds
  static unsigned long lastUploadTime = 0;
  if (millis() - lastUploadTime >= 10) {
