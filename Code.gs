  ...
  try {
    if (e && e.parameter) {
      ...
      if (e.parameter.action === 'getChartData') {
        ...
        // Any parameters received will be treated as data to save
...
function saveData(params) {
  try {
    // Ensure we have the parameters object
    if (!params) {
    ...
    if (!spreadsheet) {
      Logger.log("Could not open spreadsheet with ID: " + SHEET_ID);
      return ContentService.createTextOutput('Error: Could not open spreadsheet')
        .setMimeType(ContentService.MimeType.TEXT);
    }
    ...
    // Add the data to the sheet - using a more reliable method
    try {
      // First attempt: Try using appendRow
      sheet.appendRow(newRow);
      Logger.log('Data successfully appended to ' + MAIN_DATA_SHEET + ' using appendRow');
    } catch (appendError) {
      // Second attempt: If appendRow fails, try using setValues
      try {
        const lastRow = sheet.getLastRow();
        const range = sheet.getRange(lastRow + 1, 1, 1, newRow.length);
        range.setValues([newRow]);
        Logger.log('Data successfully appended to ' + MAIN_DATA_SHEET + ' using setValues');
      } catch (setValuesError) {
        throw new Error('Failed to append data using both methods: ' + 
                       appendError.toString() + ' and ' + setValuesError.toString());
      }
    }
    
    // Log success message
    Logger.log('Data logged successfully to ' + MAIN_DATA_SHEET + ' at ' + new Date().toISOString());
